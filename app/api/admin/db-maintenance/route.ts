  ...
  try {
    if (action === "vacuum") {
      ...
      const result = await withClient(async (c) => {
        ...
        return {
          ...
          freed_mb: Math.round((Number(before.rows[0].bytes) - Number(after.rows[0].bytes)) / 1024 / 1024),
        };
      });
    ...
    if (action === "purge-big") {
      ...
      const result = await withClient(async (c) => {
        ...
        // 2. CSV crudos de personalización procesados (los grandes)
        const r2 = await c.query(`DELETE FROM kv_store WHERE key LIKE 'csv-meta/%' AND octet_length(value::text) > 5000000`);
        deleted["csv-meta grandes"] = r2.rowCount || 0;
        // 3. Jobs de personalización viejos
        const r3 = await c.query(`DELETE FROM kv_store WHERE key LIKE 'personalization-job/%' AND octet_length(value::text) > 5000000`);
        deleted["personalization-jobs grandes"] = r3.rowCount || 0;
        return deleted;
      });
