          ...
          {connected && (
            <>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {clients.map((row) => {
                  const c = cur(row);
                  const open = openId === row.client_id;
                  return (
                    <div key={row.client_id} style={card}>
                      <div style={{ display: "flex", alignItems: "center", gap: 12, cursor: "pointer" }} onClick={() => setOpenId(open ? null : row.client_id)}>
